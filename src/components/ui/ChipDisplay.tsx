import { formatChips } from '@/lib/stripe';
...
export default function ChipDisplay({ chips, size = 'md', showIcon = true }: ChipDisplayProps) {
  const sizeClasses = {
    sm: 'text-sm',
    md: 'text-lg',
    lg: 'text-2xl'
  ...
  return (
    <div className={`chip-display ${sizeClasses[size]}`}>
      {showIcon && (
        <svg className={`${iconSizes[size]} text-poker-yellow`} fill="currentColor" viewBox="0 0 24 24">
